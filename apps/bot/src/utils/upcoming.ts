...
export async function getUpcomingTransactions(userId: string, fromDate: Date, toDate: Date): Promise<{
  ...
  const [transactionsRes, categoriesRes, loansRes, subscriptionsRes, accountsRes] = await Promise.all([
    userApi.get<any[]>(`/transactions?status=pending&sort=date_asc&limit=100&from=${fromParam}&to=${toParam}`, userId),
    userApi.get<any[]>('/categories', userId),
    userApi.get<any>('/loans/summary', userId),
    userApi.get<any>('/subscriptions/summary', userId),
    userApi.get<any[]>('/accounts', userId),
  ]);
