...
export async function showUpcoming(ctx: BotContext) {
  try {
    ...
    if (upcomingTransactions.length === 0) {
      await ctx.telegram.editMessageText(ctx.chat!.id, m.message_id, undefined, '🗓 <b>Nenhum lançamento pendente pros próximos 30 dias.</b>', { parse_mode: 'HTML' });
      return;
    }

    const totalUpcoming = upcomingTransactions.reduce((acc, t) => acc + t.amount, 0);

    let msg = `🗓 <b>Próximos Lançamentos (30 dias)</b>\n\n💰 <b>Total:</b> ${brl(totalUpcoming)}\n\n`;
    ...
    for (const t of upcomingTransactions) {
      ...
      const day = new Date(`${t.occurredAt.slice(0, 10)}T00:00:00Z`).toLocaleDateString('pt-BR', { day: '2-digit', timeZone: 'UTC' });
      ...
      msg += `${day} - ${badge}${escHtml(t.description)}${catName} <b>${brl(t.amount)}</b>\n`;
    }

    if (msg.length > 4000) {
      await ctx.telegram.editMessageText(ctx.chat!.id, m.message_id, undefined, `🗓 <b>Próximos Lançamentos (30 dias)</b>\n\n💰 <b>Total:</b> ${brl(totalUpcoming)}\n\n(A lista é muito longa, exibindo apenas as primeiras transações...)`, { parse_mode: 'HTML' });
